...
* **기댓값의 성질**:
    * **선형성 (Linearity of Expectation)**: **확률변수들의 독립 여부와 무관하게 항상 성립함.**
        1. 단일 변수의 선형 변환: $E[aX + b] = aE[X] + b \quad$ (임의의 상수 $a, b$에 대해)
        2. 여러 변수의 합 (가산성): $E[X + Y] = E[X] + E[Y] \quad$ (일반화: $E[\sum a_i X_i] = \sum a_i E[X_i]$)
    * **독립 변수 곱의 기댓값 (Product of Independent Variables)**:
        * 두 확률변수 $X, Y$가 **서로 독립일 때만 성립**하며, 선형성(Linearity)과는 구분되는 확률론적 성질임.
        * $E[XY] = E[X]E[Y]$
